...
M4aStreamReader::M4aStreamReader(InputStream *stream) {
    mStream = stream;
...
int M4aStreamReader::getSampleEncoding() {
    if (mFmtChunk->mEncodingId == M4aFmtChunkHeader::ENCODING_PCM) {
    ...
    } else if (mFmtChunk->mEncodingId == M4aFmtChunkHeader::ENCODING_IEEE_FLOAT) {
...
//                            tagStr[0], tagStr[1], tagStr[2], tagStr[3]);
        ...
        std::shared_ptr<M4aChunkHeader> chunk = nullptr;
        if (tag == M4aRIFFChunkHeader::RIFFID_RIFF) {
            chunk = mWavChunk = std::make_shared<M4aRIFFChunkHeader>(M4aRIFFChunkHeader(tag));
            mWavChunk->read(mStream);
        } else if (tag == M4aFmtChunkHeader::RIFFID_FMT) {
            chunk = mFmtChunk = std::make_shared<M4aFmtChunkHeader>(M4aFmtChunkHeader(tag));
            mFmtChunk->read(mStream);
        } else if (tag == M4aChunkHeader::RIFFID_DATA) {
            chunk = mDataChunk = std::make_shared<M4aChunkHeader>(M4aChunkHeader(tag));
            ...
            chunk = std::make_shared<M4aChunkHeader>(M4aChunkHeader(tag));
            chunk->read(mStream);
...
// Data access
void M4aStreamReader::positionToAudio() {
    if (mDataChunk != 0) {
        mStream->setPos(mAudioDataStartPos);
    }
}
 ...
 */
int M4aStreamReader::getDataFloat_PCM8(float *buff, int numFrames) {
 ...
 */
int M4aStreamReader::getDataFloat_PCM16(float *buff, int numFrames) {
    int numChannels = mFmtChunk->mNumChannels;
 ...
 */
int M4aStreamReader::getDataFloat_PCM24(float *buff, int numFrames) {
    int numChannels = mFmtChunk->mNumChannels;
 ...
 */
int M4aStreamReader::getDataFloat_Float32(float *buff, int numFrames) {
 ...
 */
int M4aStreamReader::getDataFloat_PCM32(float *buff, int numFrames) {
    int numChannels = mFmtChunk->mNumChannels;
...
int M4aStreamReader::getDataFloat(float *buff, int numFrames) {
